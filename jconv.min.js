...
jconv.convert=function(c,k,f){if(!f)throw Error("Encoding not recognized.");k=getName(k);f=getName(f);if(k===f)return c;c=ensureBuffer(c);var g=k+"to"+f;if(encodings[g])return encodings[g].convert(c);c=encodings[k+"toUTF8"].convert(c);return encodings["UTF8to"+f].convert(c)};
function getName(c){if(!c)return"UTF8";switch(c.toUpperCase()){case "CP932":case "SJIS":case "SHIFTJIS":case "SHIFT_JIS":case "SHIFT_JIS-MS":return"SJIS";case "EUCJP":case "EUC-JP":return"EUCJP";case "JIS":case "ISO2022JP":case "ISO-2022-JP":case "ISO-2022-JP-1":return"JIS";case "UTF8":case "UTF-8":return"UTF8";default:throw Error("Encoding not recognized.");}}function ensureBuffer(c){c=c||new Buffer(0);return c instanceof Buffer?c:new Buffer(c.toString(),"utf8")}
function toUtf8Buffer(c,k,f){128>c?k[f++]=c:(2048>c?k[f++]=192|c>>>6:(65536>c?k[f++]=224|c>>>12:(2097152>c?k[f++]=240|c>>>18:(67108864>c?k[f++]=248|c>>>24:(k[f++]=252|c/1073741824,k[f++]=128|c>>>24&63),k[f++]=128|c>>>18&63),k[f++]=128|c>>>12&63),k[f++]=128|c>>>6&63),k[f++]=128|c&63);return f}function toUnicodeBuffer(c,k,f){k[f++]=c&255;k[f++]=c>>8;return f}
